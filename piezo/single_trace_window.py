...
class SingleTraceWindow(QMainWindow):
    """
    简化的单条选择窗口：
    - 仅接受主程序传入的 `datasets`（list of dict，每条包含 'piezo_segment' 和 'log_G' 等字段）
    ...
    def __init__(self, keyPara, parent=None):
        ...
        self.ui.piezo_checkBox.stateChanged.connect(lambda _: self.update_plot())
    ...
    def _get_current_series(self):
        """返回当前索引对应的 (piezo, logG) 两个 ndarray（保证是 ndarray），若不存在返回两个空 ndarray。"""
        ...
        # 兼容字段名：优先 'piezo_segment' / 'log_G'，次选 'piezo' / 'log'
        piezo = np.asarray(ds.get('piezo_segment') if ds.get('piezo_segment') is not None else ds.get('piezo', []))
        ...
        try:
            ...
            if data_s_e is not None and hover_s_e is not None:
                hover_s = int(hover_s_e[0][0]) - int(data_s_e[0][0])
                hover_e = int(hover_s_e[1][0]) - int(data_s_e[0][0])
        except Exception:
            hover_s = 0
            hover_e = 0

        return piezo, logG, hover_s, hover_e

    def update_plot(self):
        piezo, logG,hover_s,hover_e = self._get_current_series()

        plot_piezo = self.ui.piezo_checkBox.isChecked()
        plot_log = self.ui.log_checkBox.isChecked()
        plot_peak = self.ui.peak_checkBox.isChecked()
        if not plot_piezo and not plot_log:
            QMessageBox.warning(self, "Warning", "Please select Piezo or Log(G/G0) to view")
            return
        ...
        if plot_piezo and piezo.size > 0:
            self.ax_right.plot(x, piezo, color='tab:blue', label='Piezo')
            self.ax_right.set_ylabel('Piezo', color='tab:blue')
    ...
    def save_selected(self):
        ...
        if self.ui.npzButton.isChecked():
            ...
            seg_events = [ds.get('segment_events', None) for ds in selected_datasets]
            piezo_list = [np.asarray(ds.get('piezo_segment') if ds.get('piezo_segment') is not None else ds.get('piezo', [])) for ds in selected_datasets]
            ...
            np.savez_compressed(save_path,
                                ...
                                segment_events=np.array(seg_events, dtype=object),
                                piezo=np.array(piezo_list, dtype=object),
            ...
            for i, ds in enumerate(selected_datasets):
                ...
                piezo_arr = np.asarray(ds.get('piezo_segment') if ds.get('piezo_segment') is not None else ds.get('piezo', []))
                ...
                # 将 piezo/log 保存为两列（长度可能不一致，较短的用 NaN 填充）
                max_len = max(piezo_arr.size if hasattr(piezo_arr, 'size') else len(piezo_arr),
                              log_arr.size if hasattr(log_arr, 'size') else len(log_arr))
                piezo_pad = np.full(max_len, np.nan, dtype=object)
                log_pad = np.full(max_len, np.nan, dtype=object)
                if piezo_arr.size > 0:
                    piezo_pad[:piezo_arr.size] = piezo_arr
                if log_arr.size > 0:
                    log_pad[:log_arr.size] = log_arr
                pd.DataFrame({'piezo': piezo_pad, 'log': log_pad}).to_csv(data_path, index=False)

                meta = {
                    'trace': trace_idx,
                    'data_s_e': ds.get('data_s_e', None),
                    'hover_s_e': ds.get('hover_s_e', None),
                    'segment_events': str(ds.get('segment_events', None)),
    ...
    def open_npz(self):
        ...
        # 支持 keys: data_s_e, hover_s_e, segment_events, piezo, logG
        piezo_list = arr['piezo'] if 'piezo' in arr else (arr['piezo_segment'] if 'piezo_segment' in arr else None)
        log_list = arr['logG'] if 'logG' in arr else (arr['log'] if 'log' in arr else None)
        n = 0
        if piezo_list is not None:
            n = len(piezo_list)
        elif log_list is not None:
            n = len(log_list)
        else:
            QMessageBox.critical(self, 'Error', 'NPZ does not contain piezo or log arrays')
            return
        ...
        for i in range(n):
            ...
            if 'segment_events' in arr:
                ds['segment_events'] = arr['segment_events'][i]

            # piezo/log 可能为 object 数组，取元素并保证为 ndarray
            if piezo_list is not None:
                ds['piezo_segment'] = np.asarray(piezo_list[i]) if piezo_list[i] is not None else np.asarray([])
